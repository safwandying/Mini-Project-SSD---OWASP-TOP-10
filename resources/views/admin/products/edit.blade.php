...
<div class="admin-wrap">
  ...
  <div>
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
      <div style="font-size:16px;font-weight:700">✏️ Edit Product</div>
      ...
      <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
          <div class="form-group" style="grid-column:1/-1">
            ...
            <input type="text" name="name" class="form-control" value="{{ old('name', $product->name) }}" required minlength="2" maxlength="200">
            @error('name')<div class="form-error">{{ $message }}</div>@enderror
          </div>

          <div class="form-group" style="grid-column:1/-1">
            <label class="form-label">Description *</label>
            <textarea name="description" class="form-control" rows="4" required minlength="10" maxlength="2000">{{ old('description', $product->description) }}</textarea>
            @error('description')<div class="form-error">{{ $message }}</div>@enderror
          </div>

          <div class="form-group">
            <label class="form-label">Sale Price (RM) *</label>
            <input type="number" name="price" class="form-control" value="{{ old('price', $product->price) }}" required step="0.01" min="0.01" max="99999.99">
            @error('price')<div class="form-error">{{ $message }}</div>@enderror
          </div>

          <div class="form-group">
            <label class="form-label">Original Price (RM) <span style="color:var(--muted);font-weight:400">(optional)</span></label>
          ...
          <div class="form-group">
            ...
            <input type="number" name="stock" class="form-control" value="{{ old('stock', $product->stock) }}" required min="0" max="100000">
            @error('stock')<div class="form-error">{{ $message }}</div>@enderror
          </div>

          <div class="form-group">
            <label class="form-label">Rating (1.0–5.0)</label>
            <input type="number" name="rating" class="form-control" value="{{ old('rating', $product->rating) }}" min="1.0" max="5.0" step="0.1">
          ...
          <div class="form-group">
            <label class="form-label">Sold Count <span style="color:var(--muted);font-weight:400">(display only)</span></label>
            <input type="number" name="sold_count" class="form-control" value="{{ old('sold_count', $product->sold_count) }}" min="0">
          </div>

          <div class="form-group" style="grid-column:1/-1">
            <label class="form-label">Product Image <span style="color:var(--muted);font-weight:400">(leave blank to keep current)</span></label>
          ...
          <div class="form-group" style="grid-column:1/-1;display:flex;align-items:center;gap:8px">
            ...
            <label for="is_active" style="font-size:13px;cursor:pointer">Active (visible to customers)</label>
          </div>
        </div>
